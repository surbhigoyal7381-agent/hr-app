...
Method path: grace_vendor_portal.goals_api.*

Bridges grace_goals (Individual Goal, Goal Cascade) and Frappe HRMS
...
def get_my_goals(include_team=0):
    ...
    hr = _is_hr()
    for g in goals:
        # Editing follows the creator, not the subject — see grace_goals.permissions.
...
# Org-tree walks live in grace_goals so the permission hooks and the portal
# agree on what "subordinate" means.
from grace_goals.permissions import (  # noqa: E402
...
def _is_hr(user=None):
    from grace_goals.permissions import FULL_ACCESS_ROLES
...
def _chain_goals(employee_id):
    ...
    scoping (grace_goals.permissions) restricts an employee to their own rows,
...
def set_goal_progress(goal_id, actual_progress):
    ...
    goal.save()
    frappe.db.commit()
    from grace_goals.controllers.goal import _update_trajectory, _aggregate_cascade
...
@frappe.whitelist()
def create_checkin(goal_id, note, progress_value=""):
    """Log a check-in note (and optional progress update) on an Individual Goal."""
    emp_id = _require_employee()
    goal = frappe.get_doc("Individual Goal", goal_id)
    ...
    if progress_value != "":
        entry.progress_value = flt(progress_value)
...
def get_checkins(goal_id):
    ...
    rows = frappe.get_all(
        ...
        fields=["name", "checkin_date", "note", "progress_value"],
    ...
    return [
        {
            ...
            "note": r["note"] or "",
            "progress_value": flt(r["progress_value"]) if r.get("progress_value") is not None else None,
        }
        for r in rows
    ]
