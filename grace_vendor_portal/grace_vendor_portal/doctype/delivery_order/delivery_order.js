            ...
            if (frm.doc.current_status === "At Location") {
                frm.add_custom_button(__("Mark Delivered"), function () {
                    frappe.confirm(
                        ...
                        function () {
                            frappe.call({
                                method: "grace_vendor_portal.controllers.delivery_order.update_delivery_status",
...
function _show_assign_partner_dialog(frm) {
    const dialog = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            dialog.hide();
            frappe.call({
                method: "grace_vendor_portal.controllers.delivery_order.assign_partner_to_order",
