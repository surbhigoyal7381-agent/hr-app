    ...
    refresh(frm) {
        ...
        if (!frm.is_new()) {
            frm.add_custom_button(__("Performance Summary"), function () {
                frappe.call({
                    method: "grace_vendor_portal.controllers.delivery_partner.get_partner_performance_summary",
            ...
            frm.add_custom_button(__("Today's Orders"), function () {
                frappe.call({
                    method: "grace_vendor_portal.controllers.delivery_partner.get_today_orders_for_partner",
                    args: { partner_name: frm.doc.name },
                    callback(r) {
                        if (!r.message) return;
