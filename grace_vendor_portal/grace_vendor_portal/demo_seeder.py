...
Run via:  bench --site hrms.localhost execute grace_vendor_portal.demo_seeder.run
"""
import frappe
...
def _advance_status(vo_name, from_status, to_status):
    ...
    for status in transitions[start:end]:
        ...
        if status == "Ready for Dispatch":
            from grace_vendor_portal.controllers.vendor_order import _prepare_delivery_pipeline
