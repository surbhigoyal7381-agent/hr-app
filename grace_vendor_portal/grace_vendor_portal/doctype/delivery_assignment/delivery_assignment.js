        ...
        if (frm.doc.status === 'Assigned' || frm.doc.status === 'In Transit') {
            frm.add_custom_button(__('Verify OTP'), function() {
                frappe.prompt([
                    ...
                    frappe.call({
                        method: 'grace_vendor_portal.controllers.delivery_assignment.verify_delivery_otp',
                        args: {
            ...
            frm.add_custom_button(__('Update GPS Location'), function() {
                frappe.prompt([
                    ...
                    frappe.call({
                        method: 'grace_vendor_portal.controllers.delivery_assignment.update_gps_location',
                        args: {
                            assignment_name: frm.doc.name,
                            lat: values.lat,
                            lng: values.lng,
