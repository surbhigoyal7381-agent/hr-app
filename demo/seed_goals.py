...
Run via:
  docker exec -i compose-backend-1 bench --site dev.alvoraa.co console < demo/seed_goals.py

Hierarchy covered:
  Rajesh Krishnamurthy (MD)
    ├── Anita Deshpande (CFO) → Alok Verma (Finance Mgr) → Sarika Bhagat (Accounts Exec)
    ├── Vikram Suryavanshi (VP Ops) → Mahesh Patil (Plant Head) → Sandeep Kulkarni / Deepak Chauhan
    ├── Priya Raghavan (HR Mgr) → Neha Kapoor (HR Exec)
    └── Suresh Iyer (Sales Mgr) → Karthik Subramanian (Sales Exec)
"""
import frappe
...
def make_goal(employee_id, goal_name, target, unit, actual_pct,
              ...
              cascade=None, parent=None, goal_type="Business", description=None):
    ...
    doc.flags.ignore_validate    = True   # skip email-send hooks
...
def make_kpi(employee_id, kpi_name, category, unit, direction,
    ...
    attainment = min(round(actual / target * 100) if target else 0, 100)
    doc = frappe.get_doc({
        "doctype":         "KPI",
        "employee":        employee_id,
        "employee_name":   _emp(employee_id),
...
cascade_doc = frappe.get_doc({
    ...
    "company":       frappe.db.get_single_value("Global Defaults", "default_company") or "Grace Drinks",
    "unit":          "Percent",
...
# Goal 1: Revenue — On Track (70%)
g_md1 = make_goal(MD,
    "Achieve ₹50 Crore Annual Revenue by Dec 2026",
...
# Goal 2: Attrition — Completed (100%)
g_md2 = make_goal(MD,
...
# Goal 3: ISO Certification — At Risk (48%)
g_md3 = make_goal(MD,
...
g_sm1 = make_goal(SM,
    "Close ₹25 Crore in Sales Revenue for H2 2026 (Jul–Dec)",
...
g_gmfg1 = make_goal(GMFG,
    ...
    target=0, unit="Number", actual_pct=100,
    status="Completed", trajectory="On Track", cascade=CASCADE, parent=g_vp2)
# Zero-target goal: actual=0 means achieved. KPI actual = 0 LTIs.
make_kpi(GMFG, "Lost-Time Injuries (Count)", "Process", "Number", "Lower is Better",
    baseline=3, target=0, actual=0, status="Achieved", goal_id=g_gmfg1)
